...
function resolveAllowlist(envSource) {
...
export function buildWorkerEnv(envSource = process.env) {
    const env = {};
...
export function assertSafeBranchName(repoDir, branchName, defaultBranch, envSource = process.env) {
...
export function shouldCreatePullRequest(envSource = process.env) {
    return envSource.FORGEFLOW_WORKER_CREATE_PR === "1";
}
export function shouldRemoveWorktreeOnExit(envSource = process.env) {
