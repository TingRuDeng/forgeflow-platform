...
function runNodeScript(scriptPath, scriptArgs, cwd, env) {
...
function normalizeChangedFiles(lines) {
    return lines
...
function main() {
    ...
    const assignmentRecord = readJson(path.join(orchestratorDir, "task-assignments.json"));
    const taskLedger = readJson(path.join(orchestratorDir, "task-ledger.json"));
    ...
    for (const assignment of filteredAssignments) {
        ...
        const workerRun = runNodeScript(runWorkerAssignmentScript, [
            "--assignment-dir",
            assignmentDir,
            "--worktree-dir",
            repoDir,
        ...
        const resultProcessing = runNodeScript(processWorkerResultScript, processArgs, process.cwd(), process.env);
        ...
        const updatedLedger = readJson(path.join(orchestratorDir, "task-ledger.json"));
    ...
    fs.writeFileSync(path.join(orchestratorDir, "execution-summary.json"), `${JSON.stringify(summary, null, 2)}\n`);
