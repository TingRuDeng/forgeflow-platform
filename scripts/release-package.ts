...
if (shouldPublish) {
  console.error(
    "Error: release-package never publishes. Use --prepare, merge the version PR, and let .github/workflows/release.yml publish from main.",
  );
  process.exit(1);
}
...
if (shouldPrepare) {
  ...
  const versionLinePattern = new RegExp(
    `^(${escapeRegExp(topLevelIndent)}"version"[\\t ]*:[\\t ]*)${escapeRegExp(currentVersionLiteral)}`,
    "m",
  );
  const updatedPackageJsonContent = packageJsonContent.replace(
    versionLinePattern,
    `$1${JSON.stringify(newVersion)}`,
  );
