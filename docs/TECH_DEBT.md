...
- dispatcher v0 start/result mutation 与 Trae 兼容 start/result 路由已支持可选 `attemptId` / `leaseToken` 校验，并会拒绝携带历史终态 `attemptId` 的 stale result；但仍未强制完整 v1 envelope
...
- Trae runtime 当前仍走现有 v0 兼容路径，但已在 `fetch-task` / `start-task` / `submit-result` 主链携带 attempt lease 字段

影响：

- 代码审查和文档阅读时容易把 vNext 目标契约误读为当前已执行的 runtime 保护
- 后续 LeaseToken enforcement 仍需要把 v0 兼容路径收紧为完整 v1 envelope，并覆盖 CLI / Console 的剩余入口
