...
- generic live executor 的 worktree Git 命令默认 60 秒超时并响应取消；有损或过长的 task 目录名会附加原始 ID 的稳定短哈希，危险目录、非法 ref、默认分支和 Git 登记错配都会 fail closed。升级前已登记在旧版无哈希精确路径的同分支 worktree 仍可复用，新任务继续使用防碰撞目录；旧路径清理必须提供并核对分支身份，符号链接路径会被拒绝。退出清理使用独立的有界 Git 操作，仍是显式 opt-in，默认保留脏 worktree，force cleanup 需要单独开启并会丢弃未提交内容。
...
- review material 现在携带 canonical `attemptId` / `artifactBundleId` / 可选 `commitSha` freshness tuple；Console、orchestrator CLI 和兼容 review CLI 会先读取 snapshot 并以 `expectedFreshness` 回传。当前 review 有 freshness 时，缺失或漂移都返回 `409`；只有历史上没有 freshness 的 review 保留兼容，成功决策把实际审查 tuple 写入证据与事件。
