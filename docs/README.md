...
- review decision 现在显式支持 `merge`、`block`、`rework`、`changes_requested`，其中后两者都会把任务落到 `blocked`，但保留原始 decision 供 redrive 和审计使用；Console 任务详情可直接提交 `merge` / `rework` / `block` 决策并刷新 dashboard snapshot，并在任务处于 `review` 时展示确定性风险分级 badge。Console 审查表单会把 `reasonCode`、`mustFix[]`、`canRedrive`、`redriveStrategy` 和高风险合并的 `acknowledgeRisk` 一起透传到 dispatcher；审查队列支持对多个 `review` 任务批量提交 `merge` / `rework` / `block` 决策，批量 merge 会要求先确认已选高风险任务，并会单独列出 `waiting_for_input` 任务供点击定位到恢复详情。
