...
- `runbooks/stage3-core-platform-operations.md`
  ...
  - shadow drift 默认 gate 保持只读；生产巡检可用 `pnpm verify:shadow-drift:reconcile` 显式对账并记录 drift alert，需要自动对账进程时使用 `pnpm verify:shadow-drift:reconciler` 周期性执行同一流程；primary-store cutover 窗口需要长期 strict 巡检时使用 `pnpm verify:shadow-cutover:reconciler`；primary-store cutover 前必须通过 `pnpm verify:shadow-cutover:drill`，该 drill 会串行执行 drift gate、reconcile + alert 记录和 strict cutover preflight；需要归档变更证据时使用 `pnpm verify:shadow-cutover:drill:evidence` 或 `--output <path>`，随后执行 `pnpm verify:shadow-cutover:approve` 生成 primary backend 所需的 approval marker，再执行 `pnpm verify:shadow-cutover:approval` 独立校验 marker、evidence SHA-256 和 revocation 状态；真正切换前执行 `pnpm verify:shadow-cutover:ready`，它会组合 strict preflight 和 approval evidence 两个 phase；primary backend 会在连接前校验 marker、`evidencePath` 和 `evidenceSha256`；回滚或撤销切换窗口时执行 `pnpm verify:shadow-cutover:revoke` 归档 approval marker 并生成 revocation marker。
