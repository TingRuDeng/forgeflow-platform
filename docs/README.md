...
- dispatcher 任务状态机现在包含 `cancelled` 和 `waiting_for_input`；控制面和 console 都可以显式作废非终态任务，控制面可通过 `/api/tasks/:taskId/interrupt` / `/api/tasks/:taskId/resume` 暂停任务、保存 `resumePayload` 并恢复到 `ready`。
