...
- dashboard snapshot 现在暴露 `taskAttempts` 与 `artifactBundles`，Console 任务详情可直接查看 attempt timeline、runtime events、artifact summary、refs、retained content 和结构化 trajectory；dispatcher 会把 retained diff / log / test result / trajectory 写入本地 artifact store，并通过 artifact 文件 API 和 `artifact-get --file <name>` 按需读取。
