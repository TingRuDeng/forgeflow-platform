...
5. 相关契约：
   ...
   - `contracts/execution-profile-v1.md`
6. 对应模块入口：
   - `../apps/dispatcher/README.md`
   - `../scripts/lib/README.md`
7. 对应 `scripts/`、`apps/dispatcher/`、`packages/`
...
- Phase 1 运行时合并到 TypeScript 已完成；当前主链入口仍在 `scripts/*.js`，但 `worker-daemon`、`review-decision`、`dispatcher-state`、`dispatcher-server` 已桥接到 `apps/dispatcher/dist` 的 TypeScript foundation；`review-memory` 权威实现位于 `apps/dispatcher`，task worktree 权威实现已统一到 `@tingrudeng/beta-runtime-core`。
...
- `worker-daemon` / Trae runtime 现在只有在结果成功回写到 dispatcher 后才会对外呈现“完成”；`submitResult`、`git push`、自动 PR 创建失败都属于显式失败，而不是假完成。Trae terminal result 与 shared daemon cycle 默认都最多尝试 3 次、间隔 2 秒，可用 `WORKER_DAEMON_SUBMIT_RESULT_MAX_RETRIES` / `WORKER_DAEMON_SUBMIT_RESULT_RETRY_DELAY_MS` 调整；Trae 在结果未获确认时保留 session 与 worktree，在当前执行退栈时释放本机 worktree owner，停止任务轮询并以非零状态退出。generic / Trae progress 复用共享有界投递策略，同一更新固定复用 `progressId`，仅对网络/超时、`408`、`425`、`429` 和 `5xx` 默认总计尝试 3 次、间隔 1 秒，可用 `WORKER_PROGRESS_MAX_ATTEMPTS` / `WORKER_PROGRESS_RETRY_DELAY_MS` 调整；普通 `4xx` 立即失败。generic HTTP / state-dir、Trae automation、packaged Trae runtime 与 Trae MCP heartbeat 复用同一错误分类，固定最多尝试 4 次、间隔 1 秒；generic daemon 与 Trae automation heartbeat 保持 single-flight，停止或切换模式会取消当前请求或重试等待。assignment 子进程默认总超时为 30 分钟，可用 `WORKER_DAEMON_EXECUTION_TIMEOUT_MS` 覆盖。执行 timeout / SIGINT / SIGTERM 会取消 dispatcher HTTP 请求与重试等待，并终止完整子进程树。
- Codex、Gemini 与 Trae 的 worktree 操作统一由 `@tingrudeng/beta-runtime-core` 提供：Git 命令默认 60 秒超时并响应取消；有损或过长的 task 目录名会附加原始 ID 的稳定短哈希，危险目录、非法 ref、默认分支和 Git 登记错配都会 fail closed。复用时统一执行 `reset --hard` + `clean -fd`。同一 Git common-dir 下的 ForgeFlow 生命周期共用本机 owner lock，默认等待 2 秒，活跃 PID 不会仅因锁变旧而被回收；generic worker 持锁到 assignment 与 Git 收尾完成，Trae 持锁到结果回写与可选清理完成。可选清理默认非 force 并保留脏 worktree，`FORGEFLOW_WORKER_FORCE_WORKTREE_CLEANUP=1` 才会丢弃未提交内容；该锁不覆盖跨主机、独立 clone 或直接手工 Git。
...
- review decision 现在显式支持 `merge`、`block`、`rework`、`changes_requested`，其中后两者都会把任务落到 `blocked`，但保留原始 decision 供 redrive 和审计使用；Console 任务详情可直接提交 `merge` / `rework` / `block` 决策并刷新 dashboard snapshot，并在任务处于 `review` 时展示确定性风险分级 badge。Console 审查表单会把 `reasonCode`、`mustFix[]`、`canRedrive`、`redriveStrategy` 和高风险合并的 `acknowledgeRisk` 一起透传到 dispatcher；审查队列支持对多个 `review` 任务批量提交 `merge` / `rework` / `block` 决策，批量 merge 会要求先确认已选高风险任务，批量提交结果会在队列内展示成功数和逐项失败明细，并会单独列出 `waiting_for_input` 任务供点击定位到恢复详情。
- review material 必须携带 canonical `attemptId` / `artifactBundleId` / 可选 `commitSha` freshness tuple；Console、orchestrator CLI 和兼容 review CLI 会先读取 snapshot 并以 `expectedFreshness` 回传。请求缺失该字段返回 `400`，tuple 漂移返回 `409`；成功决策把实际审查 tuple 写入证据与事件。旧状态若没有 freshness，必须重新执行或 redrive 生成当前审查材料，不能无 fence 决策。
- dispatcher 使用共享规则把 worker result 的首个结构化 blocker（或兼容 fallback）同步写入终态 event 与 TaskAttempt，并为 snapshot task 派生权威 `redriveEligibility`；Console 会从 worker result、attempt 和 runtime event 中按时间选择最新失败，并允许可恢复的 `failed` 或 `blocked + rework` 任务通过幂等 `/api/tasks/:taskId/redrive` 创建新 `-rN` 分支与 continuation lineage。
- `forgeflow-review-orchestrator decide` 现在支持 `--reason-code`、`--must-fix`、`--can-redrive`、`--redrive-strategy`，并会把这些字段归一化写入 `review.evidence`。
- `forgeflow-review-orchestrator decide --state-dir` 通过本地 dispatcher bridge 执行同一 review decision route，默认读写 SQLite 真相源并保留完整风险门、lease release 与审计事件语义。
- `forgeflow-review-orchestrator` 的 `inspect --summary` / `watch --summary` 现在会带上 review 风险分级（`riskAssessment.level/reasons/changedFileCount/protectedPathHits`）；`decide --decision merge` 现在受该分级软门禁：读取到非 `low` 风险时 fail closed，需显式 `--acknowledge-risk` 覆盖；只有风险字段缺失时才放行，且只约束 merge。dispatcher 另提供服务端权威门禁 `DISPATCHER_REVIEW_MERGE_GATE`（默认 `off`，`enforce` 时风险非 `low` 的 merge 返回 `409`，`warn` 放行并记 `review_merge_risk_acknowledged`），覆盖 Console / CLI / 直连 HTTP。
...
- busy worker heartbeat 会同步续租 active attempt 与 task resource leases；dispatcher reconcile 会扫描所有过期 running attempt，不因 worker 仍在线而越过 lease 边界，并按默认最多 2 次 attempt 的最小策略自动 redrive 或把任务显式置为 `failed`。
...
- vNext runtime reliability 的目标契约已落地到 `@forgeflow/worker-protocol`，当前 dispatcher worker start/progress/result mutation 已强制完整 `TaskAttempt` / `LeaseToken` envelope；`@tingrudeng/codex-beta-runtime` 和 `@tingrudeng/gemini-beta-runtime` 会通过 `POST /api/workers/:workerId/claim-task` 领取任务，并把 dispatcher 返回的 `attemptId`、`leaseToken`、`protocolVersion`、`traceId`、`idempotencyKey` 作为 start/progress/result envelope 回写。Trae progress 兼容入口执行相同 fence，通用 telemetry endpoint 不再接受 progress 事件。该包还提供 start/progress/result payload helper 作为内部 Worker SDK 契约。
...
- identity-aware HITL 会持久化 `requestId`、可选 active `attemptId` / `sourceSessionId` / `expiresAt` 和最近已处理输入。Console 恢复会回传 request/attempt identity；缺失、错配或过期请求返回 `409`，相同 identity 与 payload 的重试幂等成功，改变 payload 的重放被拒绝。旧的无 identity 持久化请求仍兼容恢复。
...
- Trae terminal result 投递失败会写 `submit_result_retry_failed`，耗尽后写 `delivery_failed`；只有 dispatcher 确认终态后才把 worker 降为 idle 并释放 session。耗尽时 runtime 会保留 session 与 worktree、释放当前执行的本机 owner、停止任务轮询并退出，避免把未确认任务再次取回执行。
...
- `../.github/workflows/ci.yml`
  ...
  - 第三方 action 固定到完整 commit SHA，workflow 默认只授予 `contents: read`；`.github/dependabot.yml` 每周检查 GitHub Actions pin 更新。
...
- `runbooks/stage3-core-platform-operations.md`
  ...
  - shadow drift 默认 gate 保持只读；生产巡检可用 `pnpm verify:shadow-drift:reconcile` 显式对账并记录 drift alert，需要自动对账进程时使用 `pnpm verify:shadow-drift:reconciler` 周期性执行同一流程，并默认写入 `.forgeflow-dispatcher/shadow-reconciler-status.json` 的 `shadow-reconciler-status/v1` 状态快照；primary-store cutover 窗口需要长期 strict 巡检时使用 `pnpm verify:shadow-cutover:reconciler`，它也会更新同一状态文件；primary-store cutover 前必须通过 `pnpm verify:shadow-cutover:drill`，该 drill 会串行执行 drift gate、reconcile + alert 记录和 strict cutover preflight；需要归档变更证据时使用 `pnpm verify:shadow-cutover:drill:evidence` 或 `--output <path>`，随后执行 `pnpm verify:shadow-cutover:approve` 生成 primary backend 所需的 approval marker，再执行 `pnpm verify:shadow-cutover:approval` 独立校验 marker、evidence SHA-256 和 revocation 状态；真正切换前执行 `pnpm verify:shadow-cutover:ready:evidence`，它会组合 strict preflight 和 approval evidence 两个 phase 并归档 ready evidence；切换后执行 `pnpm verify:shadow-cutover:complete:evidence` 归档 Postgres primary snapshot 可读证据；primary backend 会在连接前校验 marker、`evidencePath`、`evidenceSha256` 和 ready evidence；`/api/dr/status.primaryCutover` 与 Console DR 状态面板会显示 approval / ready / completion / revocation evidence 是否 ready 或 completed，Console 也会展示 shadow reconciler 最近一轮 request / attempt / reason 证据；回滚或撤销切换窗口时执行 `pnpm verify:shadow-cutover:revoke` 归档 approval marker 并生成 revocation marker。
