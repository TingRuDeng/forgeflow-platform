...
- `GET /api/dr/status`
  ...
  - `primaryCutover` 会读取 stateDir 下的 `shadow-cutover-approval.json`、`shadow-cutover-ready.json`、`shadow-cutover-complete.json` 和 `shadow-cutover-revocation.json`，返回 `unknown` / `ready` / `completed` / `blocked` / `failed`；ready 状态要求 approval marker、archived drill evidence SHA-256 和 final ready evidence 完全匹配；completed 状态还要求 completion evidence 绑定同一 approval marker 并证明 Postgres primary snapshot 可读；revocation marker 存在时返回 `blocked`。
  ...
  - `DISPATCHER_SHADOW_MODE=primary` 仅在 `RUNTIME_STATE_BACKEND=postgres` 且 `DISPATCHER_PRIMARY_POSTGRES_URL` 已配置时视为切换后兼容状态；此时 shadow drift / cutover 检查会返回 matched。若 primary backend 未选择，runtime shadow 写入会显式记录 `primary_backend_not_selected`。
