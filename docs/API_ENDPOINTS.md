...
- `GET /api/dr/status`
  ...
  - `primaryCutover` 会读取 stateDir 下的 `shadow-cutover-approval.json`、`shadow-cutover-ready.json` 和 `shadow-cutover-revocation.json`，返回 `unknown` / `ready` / `blocked` / `failed`；ready 状态要求 approval marker、archived drill evidence SHA-256 和 final ready evidence 完全匹配，revocation marker 存在时返回 `blocked`。
