...
- 切换进程环境到 `RUNTIME_STATE_BACKEND=postgres` 并完成至少一次 dispatcher primary snapshot 写入后，执行 `pnpm verify:shadow-cutover:complete:evidence` 归档 `.forgeflow-dispatcher/shadow-cutover-complete.json`；该命令会复核 ready evidence、primary backend 环境和 Postgres `dispatcher_runtime_state` snapshot 可读性。`/api/dr/status.primaryCutover` 只会在 completion evidence 内的 ready evidence 与当前 approval marker 完全匹配时显示 `completed`
