...
function hasActionableRecommendation(recommendation: string): boolean {
  ...
  return actionablePatterns.some((pattern) =>
    recommendation.toLowerCase().includes(pattern)
  );
}
...
function isExtractableFromFailed(evidence: WorkerEvidence): boolean {
  ...
  return abstractablePatterns.some((pattern) => errorText.includes(pattern));
}
...
function matchScope(criteriaScope: string, lessonTriggerPaths: string[]): boolean {
  ...
  return lessonTriggerPaths.some((candidatePath) => {
    const normalizedPath = candidatePath.replace(/\*\*$/, "").replace(/\/$/, "");
