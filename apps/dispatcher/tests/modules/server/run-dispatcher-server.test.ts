...
describe("run-dispatcher-server.js CLI", () => {
  ...
  it("defaults to legacy auth mode when no argument provided", async () => {
    const { parseArgs } = await loadCliModule();
    const args = parseArgs([]);
    expect(args.authMode).toBe("legacy");
  });
  ...
  it("uses token auth mode when --auth-mode token is provided", async () => {
    const { parseArgs } = await loadCliModule();
    const args = parseArgs(["--auth-mode", "token"]);
    expect(args.authMode).toBe("token");
  });

  it("uses legacy auth mode when --auth-mode legacy is provided", async () => {
    const { parseArgs } = await loadCliModule();
    const args = parseArgs(["--auth-mode", "legacy"]);
    expect(args.authMode).toBe("legacy");
  });
  ...
  it("writes DISPATCHER_AUTH_MODE=legacy by default", async () => {
    const { applyAuthMode, parseArgs } = await loadCliModule();
    const previous = process.env.DISPATCHER_AUTH_MODE;
    const args = parseArgs([]);
    applyAuthMode(args);

    expect(process.env.DISPATCHER_AUTH_MODE).toBe("legacy");
