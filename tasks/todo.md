...
已新增 Console Artifact Workbench 级的跨任务 trajectory 并排详情区块。Workbench 现在会在筛选后的 artifact bundles 中收集带结构化 trajectory 的 bundle，按任务卡片并排展示每个 step 的 phase、status、action、observation、command 和 artifactRef，并支持从该区块点击回对应任务。这样审查者可以在跨任务视图中直接比较执行轨迹，不必只依赖每张卡片的轨迹摘要或进入单任务详情页。

Review Gate：finished。Spec 符合度通过，当前实现只增强 Console Artifact Workbench 的前端展示，不改变 dispatcher artifact store、trajectory schema、artifact 文件 API、任务状态机或 worker result 合约。安全检查通过，未新增 secret、外部命令、网络写入、mock 成功路径或静默 fallback。复杂度检查通过，新增 `ArtifactWorkbenchTrajectoryComparison.tsx` 85 行，`ArtifactWorkbench.tsx` 255 行。Document-refresh: needed，原因：Console 审查工作台可见能力变化，已同步 README、docs README、TECH_DEBT 和 tasks。结论：通过。

验证已通过：`CI=true pnpm --filter console exec vitest run src/components/__tests__/Lists.test.tsx src/__tests__/App.test.tsx --maxWorkers=1`，2 个测试文件、22 个测试通过；`CI=true pnpm --filter console lint`；`CI=true pnpm --filter console build`；`CI=true pnpm typecheck`；`CI=true pnpm lint`；`CI=true pnpm docs:validate`；`python3 scripts/validate_docs.py . --profile generic`；`git diff --check`；提升权限运行 `CI=true pnpm test`，全量测试通过，其中 Console 7 个测试文件、36 个测试通过，dispatcher 60 个测试文件、548 个测试通过。

剩余风险：Workbench 已能跨任务并排查看结构化 trajectory steps；按 step 展开 / 下载 artifactRef 正文仍在单任务详情 trajectory tab 和卡片 refs 区完成，Workbench 级 trajectory step artifact 文件内联展开不是本轮范围。

## M102 Review 小结

已让 Console Artifact Workbench 的跨任务搜索索引纳入 trajectory step 的 `phase`、`action`、`observation`、`status`、`command`、`cwd`、`exitCode` 和 `artifactRef`。Artifact 卡片现在会展示轨迹步骤数、失败步骤数和最后一步摘要，审查者可以直接按执行命令、观察文本或轨迹 artifact 引用定位任务，不必先进入单任务详情页。

Review Gate：finished。Spec 符合度通过，当前实现只增强 Console Artifact Workbench 的索引和摘要展示，不改变 dispatcher artifact store、trajectory schema、artifact 文件 API、任务状态机或 worker result 合约。安全检查通过，未新增 secret、外部命令、网络写入、mock 成功路径或静默 fallback。复杂度检查通过，新增 `ArtifactWorkbenchTrajectory.tsx` 27 行、`artifactTrajectorySummaryModel.ts` 38 行。Document-refresh: needed，原因：Console 审查工作台可见能力变化，已同步 README、docs README、TECH_DEBT 和 tasks。结论：通过。

验证已通过：`CI=true pnpm --filter console exec vitest run src/components/__tests__/Lists.test.tsx src/__tests__/App.test.tsx --maxWorkers=1`，2 个测试文件、22 个测试通过；`CI=true pnpm --filter console lint`；`CI=true pnpm --filter console build`；`CI=true pnpm typecheck`；`CI=true pnpm lint`；`CI=true pnpm docs:validate`；`python3 scripts/validate_docs.py . --profile generic`；`git diff --check`；提升权限运行 `CI=true pnpm test`，全量测试通过，其中 Console 7 个测试文件、36 个测试通过，dispatcher 60 个测试文件、548 个测试通过。

剩余风险：Workbench 现在能跨任务检索和摘要 trajectory；完整逐步回放仍在单任务详情 trajectory tab 内完成，跨任务多轨迹并排回放仍不是本轮范围。
...
已把 Trae launch target 构建继续下沉到 `packages/automation-gateway-core/src/trae-launch-target.ts`。共享 core 现在统一提供 `parseLaunchArgs`、`hasRemoteDebuggingPortArg` 和 `resolveTraeLaunchTarget`，覆盖 launch args 解析、remote debugging port 注入、project path 注入和 macOS `.app` executable 解析。`scripts/lib/trae-launcher.ts` 与 `packages/trae-beta-runtime/src/runtime/trae-launcher.ts` 继续保留原公开导出，但实现改为委托 `@tingrudeng/automation-gateway-core`，避免源码脚本和 packaged runtime 在 target 组装规则上继续漂移。

Review Gate：finished。Spec 符合度通过，本轮继续推进 runtime executor / launch 去重，只迁移 target 构建规则，不改变 Trae spawn、debugger wait、clean relaunch、gateway、worker、dispatcher API 或 session-store。安全检查通过，共享 helper 只解析参数、读取本地 `.app` bundle 并显式校验 project path；未新增 secret、外部命令、网络写入、mock 成功路径或静默 fallback。复杂度检查通过，共享 target helper、脚本 launcher 和 packaged launcher 均保持单一职责拆分。Document-refresh: needed，原因：`automation-gateway-core` 共享职责新增完整 launch target 构建，已同步 README、TECH_DEBT、package README 和 tasks。结论：通过。

验证已通过：`CI=true pnpm --filter @tingrudeng/automation-gateway-core exec vitest run tests/trae-launch-target.test.ts tests/trae-clean-relaunch.test.ts --maxWorkers=1`，2 个测试文件、11 个测试通过；`CI=true pnpm --filter @tingrudeng/automation-gateway-core typecheck`；`CI=true pnpm --filter @tingrudeng/automation-gateway-core build`；`CI=true pnpm --dir scripts/lib exec tsc -p tsconfig.json`；`CI=true pnpm --filter @forgeflow/dispatcher exec vitest run tests/modules/server/trae-launcher.test.ts --maxWorkers=1`，1 个测试文件、6 个测试通过；`CI=true pnpm --filter @tingrudeng/trae-beta-runtime exec vitest run tests/trae-launcher.test.ts --maxWorkers=1`，1 个测试文件、2 个测试通过；`CI=true pnpm --filter @tingrudeng/trae-beta-runtime typecheck`；`CI=true pnpm --filter @tingrudeng/trae-beta-runtime build`；`CI=true pnpm typecheck`；`CI=true pnpm lint`；`CI=true pnpm docs:validate`；`python3 scripts/validate_docs.py . --profile generic`；`git diff --check`；`node --check scripts/lib/trae-launcher.js`；`node --check packages/automation-gateway-core/dist/trae-launch-target.js`；`node --check packages/trae-beta-runtime/dist/runtime/trae-launcher.js`；提升权限运行 `CI=true pnpm test`，全量测试通过，其中 automation-gateway-core 8 个测试文件、30 个测试通过，trae-beta-runtime 20 个测试文件、175 个测试通过，dispatcher 60 个测试文件、548 个测试通过。

剩余风险：Trae launch target 构建已共享；Trae launch 的 spawn / wait debugger 编排仍在 scripts launcher 与 packaged launcher 各自保留，后续继续去重时应单独评估是否迁入共享 core。

## M99 Review 小结

已新增 `packages/automation-gateway-core/src/trae-launch-target.ts`，把 macOS `.app` 到 `Contents/MacOS/*` 可执行文件的解析规则下沉为共享 `resolveMacAppBundleExecutable`。`scripts/lib/trae-launcher.ts` 和 `packages/trae-beta-runtime/src/runtime/trae-launcher.ts` 删除本地重复实现，改为从 `@tingrudeng/automation-gateway-core` 引用并继续 re-export 兼容入口。这样源码脚本与 packaged runtime 的 `.app` executable 探测规则不再漂移。

Review Gate：finished。Spec 符合度通过，本轮继续推进 runtime executor / launch 去重，只迁移 `.app` executable 解析，不改变 CLI 参数、remote debugging port 注入、project path 注入、spawn/wait debugger、gateway、worker、dispatcher API 或 session-store。安全检查通过，共享 helper 只读取本地 bundle 目录并跳过不可读取条目；未新增 secret、外部命令、网络写入、mock 成功路径或静默 fallback。复杂度检查通过，`trae-launch-target.ts` 52 行、测试 80 行、`scripts/lib/trae-launcher.ts` 降为 264 行、packaged launcher 降为 207 行。Document-refresh: needed，原因：`automation-gateway-core` 共享职责新增 launch target helper，已同步 README、TECH_DEBT、package README 和 tasks。结论：通过。

验证已通过：`CI=true pnpm --filter @tingrudeng/automation-gateway-core exec vitest run tests/trae-launch-target.test.ts tests/trae-clean-relaunch.test.ts --maxWorkers=1`，2 个测试文件、8 个测试通过；`CI=true pnpm --filter @tingrudeng/automation-gateway-core typecheck`；`CI=true pnpm --filter @tingrudeng/automation-gateway-core build`；`CI=true pnpm --dir scripts/lib exec tsc -p tsconfig.json`；`CI=true pnpm --filter @forgeflow/dispatcher exec vitest run tests/modules/server/trae-launcher.test.ts --maxWorkers=1`，1 个测试文件、6 个测试通过；`CI=true pnpm --filter @tingrudeng/trae-beta-runtime exec vitest run tests/trae-launcher.test.ts --maxWorkers=1`，1 个测试文件、2 个测试通过；`CI=true pnpm --filter @tingrudeng/trae-beta-runtime typecheck`；`CI=true pnpm --filter @tingrudeng/trae-beta-runtime build`；`CI=true pnpm typecheck`；`CI=true pnpm lint`；`CI=true pnpm docs:validate`；`python3 scripts/validate_docs.py . --profile generic`；`git diff --check`；`node --check scripts/lib/trae-launcher.js`；`node --check packages/automation-gateway-core/dist/trae-launch-target.js`；`node --check packages/trae-beta-runtime/dist/runtime/trae-launcher.js`；提升权限复跑 `CI=true pnpm test`，全量测试通过，其中 automation-gateway-core 8 个测试文件、27 个测试通过，dispatcher 60 个测试文件、548 个测试通过。首次全量测试中 `trae-automation-worker.test.ts` 有 1 个 prompt 构建用例 20s 超时；按 systematic-debugging 单独复现该用例 1.1s 通过、完整文件 13 个测试 1.93s 通过，全量复跑通过，判断为并发全量测试抖动。

剩余风险：`.app` executable 解析和 clean relaunch 原语已共享；Trae launch 的完整 target 构建、参数解析、spawn/wait debugger 编排仍在 scripts launcher 与 packaged launcher 各自保留，后续若要继续压缩 launch 重复，需要评估是否把完整 `resolveTraeLaunchTarget` 或 spawn wait 编排迁入 shared core。
