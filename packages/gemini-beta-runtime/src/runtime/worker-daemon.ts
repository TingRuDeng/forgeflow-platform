...
function runWorkerAssignmentScript(packageRoot: string, assignmentDir: string, worktreeDir: string, outputDir: string): Promise<WorkerResult> {
  return new Promise((resolve, reject) => {
    const scriptPath = path.join(packageRoot, "dist/runtime/run-worker-assignment.js");
...
async function processTaskAssignment(input: ProcessTaskAssignmentInput): Promise<{
  ...
  try {
    ...
    const workerResult = input.dryRunExecution
      ...
      : await runWorkerAssignmentScript(input.packageRoot, assignmentDir, worktreeDir, outputDir);
