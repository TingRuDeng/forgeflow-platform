...
describe("@tingrudeng/gemini-beta-runtime update", () => {
  it("defaults global updates to the beta dist-tag", async () => {
      ...
      if (args[0] === "list") {
        return {
          stdout: JSON.stringify({
            dependencies: {
              "@tingrudeng/gemini-beta-runtime": { version: "0.1.0-beta.3" },
            },
          }),
          stderr: "",
        };
    ...
    expect(execFile).toHaveBeenCalledWith("npm", [
      "install",
      "-g",
      "@tingrudeng/gemini-beta-runtime@beta",
    ]);
  });
  ...
  it("keeps explicit dist-tag overrides available", async () => {
    ...
    expect(result.performedCommand).toBe(
      "npm install -g @tingrudeng/gemini-beta-runtime@next",
    );
  });
});
