...
describe("gemini beta runtime cli", () => {
  ...
  it("parses gemini-bin option", () => {
    ...
    expect(parsed.command).toBe("start");
    expect(parsed.subcommand).toBe("worker");
    expect(parsed.options.geminiBin).toBe("/usr/local/bin/gemini");
  });
