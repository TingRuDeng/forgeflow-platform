...
export function buildDiscoveryConfig(options = {}) {
  return {
...
export function buildDebuggerBaseUrl(options = {}) {
  const config = buildDiscoveryConfig(options);
  return `http://${config.host}:${config.port}`;
}

export async function fetchDebuggerJson(pathname, options = {}) {
  const config = buildDiscoveryConfig(options);
...
function isWorkbenchPageTarget(target) {
  if (!isInspectablePageTarget(target)) {
    return false;
  }
  const url = String(target.url || "").toLowerCase();
  return url.includes("/workbench/workbench.html");
}
...
export function selectTraeTarget(targets = [], options = {}) {
  const config = buildDiscoveryConfig(options);
  ...
  if (rankedTargets.length === 0) {
    const pageTargets = Array.isArray(targets)
      ? targets.filter((target) => isWorkbenchPageTarget(target))
      : [];
...
export async function getDebuggerVersion(options = {}) {
  const config = buildDiscoveryConfig(options);
  const version = await fetchDebuggerJson("/json/version", options);
  return {
    ...version,
    host: config.host,
    port: config.port,
...
export async function listDebuggerTargets(options = {}) {
  return fetchDebuggerJson("/json/list", options);
}

export async function discoverTraeTarget(options = {}) {
  const config = buildDiscoveryConfig(options);
