...
import type { CodexBetaConfig, CodexBetaDoctorCheck, CodexBetaDoctorResult } from "./types.js";
...
export function runCodexBetaDoctor(options: {
  configPath?: string;
  cwd?: string;
  ...
  const checks: CodexBetaDoctorCheck[] = [];
  ...
  checks.push({
    name: "codex-bin",
    ...checkCommand(config.codexBin, ["--version"]),
  });
  ...
  checks.push({
    name: "pool",
    ok: config.pool === "codex",
    message: config.pool === "codex" ? "pool is codex" : "pool must be codex",
  });
...
export function formatCodexBetaDoctorResult(result: CodexBetaDoctorResult): string {
  const lines = [
