...
export async function updateLocalCheckout(options: UpdateOptions = {}): Promise<UpdateResult> {
  const packageName = "@tingrudeng/codex-beta-runtime";
  ...
  try {
    ...
    return {
      ...
      message: "Updated the global ForgeFlow Codex beta runtime package. Restart long-running worker processes to use the new version.",
    };
  } catch (error) {
