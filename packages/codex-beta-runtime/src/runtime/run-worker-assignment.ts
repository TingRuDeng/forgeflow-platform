...
const CODEX_BIN_OVERRIDE = process.env.FORGEFLOW_CODEX_BIN?.trim() || "";
const CODEX_SANDBOX = process.env.FORGEFLOW_CODEX_SANDBOX?.trim() || "workspace-write";
const CODEX_ARGS_JSON = process.env.FORGEFLOW_CODEX_ARGS_JSON?.trim() || "";
const CODEX_ARGS_TEXT = process.env.FORGEFLOW_CODEX_ARGS?.trim() || "";
...
function parseCodexExtraArgs(): string[] {
  if (CODEX_ARGS_JSON) {
    try {
      const parsed = JSON.parse(CODEX_ARGS_JSON) as unknown;
  ...
  return splitArgs(CODEX_ARGS_TEXT);
}
...
function resolveCodexBin(): string {
  ...
  const candidates = CODEX_BIN_OVERRIDE
    ? [CODEX_BIN_OVERRIDE]
    : ["codex"];
  ...
  throw new Error(
    "codex binary not found. Install codex CLI or set FORGEFLOW_CODEX_BIN to an executable path.",
  );
}

function buildCodexExecArgs(prompt: string): string[] {
  const codexBin = resolveCodexBin();
  const argv = [codexBin, "exec"];
  if (CODEX_MODEL) {
...
function buildLaunchCommand(assignment: Assignment, prompt: string, worktreeDir: string): LaunchCommand {
  if (assignment.pool !== "codex") {
    throw new Error(`codex runtime only supports pool=codex, got ${assignment.pool}`);
  }

  return {
    provider: "codex",
    argv: buildCodexExecArgs(prompt),
    cwd: worktreeDir,
  };
}
