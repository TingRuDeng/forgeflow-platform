...
- Console 任务详情现在可提交结构化审查证据：`reasonCode`、`mustFix[]`、`canRedrive`、`redriveStrategy`，并在风险非 `low` 时把人工确认随 `acknowledgeRisk` 透传到 dispatcher；审查队列支持对多个 `review` 任务批量提交 `merge` / `rework` / `block` 决策，批量 merge 会要求先确认已选高风险任务，并会单独列出 `waiting_for_input` 任务供点击定位到恢复详情。
