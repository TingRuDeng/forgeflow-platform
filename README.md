...
- 一个最小的远程机器运行时包已在 `packages/trae-beta-runtime/` 下进入 beta 安装路径，用来提供 launch/gateway/worker/update 命令；当前 npm 包版本是 `@tingrudeng/trae-beta-runtime@0.1.0-beta.39`，`update` 会直接自更新已安装包，不是推荐式卸载重装流程。
...
如需启用认证保护，使用 `DISPATCHER_AUTH_MODE=token`：
...
- `token`：需要 `DISPATCHER_API_TOKEN`，所有接口（除 `/health`）需要 `Authorization: Bearer <token>`
- `open`：显式开放模式，无需认证，用于本地开发或可信网络
