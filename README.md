...
- review memory 已有文件存储契约，并会在 dispatcher 创建 dispatch 时按 repo/scope/worker 条件注入上下文。
  - 支持从结构化 worker result (failureType/blockers/findings/artifacts) 提取 lesson
  - 支持从结构化 review decision (reasonCode/mustFix/canRedrive) 提取 lesson
  - 保留旧版自由文本 fallback 兼容性
