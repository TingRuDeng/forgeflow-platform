...
- 阶段三核心底座现在已进入主线：dispatcher 运行时状态引入显式 `leases[]`，assignment / session / repo / branch ownership 都能做冲突检测、过期回收和指标聚合。
...
- `DISPATCHER_READ_ONLY_MODE=1` 现在是正式只读降级开关；查询面继续可用，状态型 HTTP 写入统一返回 `503 read_only_mode`。
- 仓库现在内置阶段三参考部署和演练入口：`pnpm verify:stage3`、`scripts/backup-runtime-state.mjs`、`scripts/restore-runtime-state.mjs`、`scripts/verify-stage3-dr.mjs`、`deploy/compose/*`、`deploy/helm/forgeflow/*`。
