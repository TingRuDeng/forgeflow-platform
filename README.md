...
forgeflow-platform 是多智能体协作开发的控制平面仓库。当前主线以 `dispatcher` 为任务、分配、状态流转与审计记录的真相源；`codex-control` 等控制层负责编排；`codex`、`gemini`、`Trae` 都是受支持、并列的 worker 接入方式。源码中三者均有远程运行时包；npm registry 当前公开可安装的是 `@tingrudeng/codex-beta-runtime` 与 `@tingrudeng/trae-beta-runtime`，`@tingrudeng/gemini-beta-runtime` 仍需先完成 npm 包名和 Trusted Publisher 配置。说明：Trae automation 仍是最成熟的无人值守链路；`codex/gemini` worker daemon 的主循环、worker CLI、executor、launch builder 和失败回写已收敛到 `@tingrudeng/beta-runtime-core`，dispatcher runtime-glue 和源码脚本只保留薄适配；`pnpm verify:runtime-packages` 会校验 runtime 包发布清单，显式加 `--check-registry --require-published` 时可把 npm registry 创建状态变成生产发布硬门禁，但 Gemini 远程包发布仍依赖外部 npm 配置（见 `docs/TECH_DEBT.md`）。
