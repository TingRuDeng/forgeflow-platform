...
- Console 任务详情现在可提交结构化审查证据：`reasonCode`、`mustFix[]`、`canRedrive`、`redriveStrategy`，并在风险非 `low` 时把人工确认随 `acknowledgeRisk` 透传到 dispatcher；审查队列支持对多个 `review` 任务批量提交 `rework` / `block` 决策。
