...
- `forgeflow-review-orchestrator` 现在补齐了阶段二控制层闭环：`dispatch/dispatch-task/watch/inspect/decide` 都支持 `--state-dir` 本地 dispatcher fallback；`watch --summary` 与 `inspect --summary` 也会统一输出 review/failure/redrive/progress 摘要。
...
- Trae runtime 现在会通过 `POST /api/workers/:workerId/events` 回写结构化 phase events 与 failure blocker codes，控制层不必再只靠 regex 猜测失败类型。
...
- `scripts/lib/*.js` 仍是 live adapter 层，不属于这轮精简范围。
