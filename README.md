...
- 一个最小的远程机器运行时包已在 `packages/trae-beta-runtime/` 下进入 beta 安装路径，用来提供 launch/gateway/worker/update 命令；当前 npm 包版本是 `@tingrudeng/trae-beta-runtime@0.1.0-beta.38`，`update` 会直接自更新已安装包，不是推荐式卸载重装流程。
