...
- ForgeFlow 是 Trae-first 的多智能体协作开发控制平面，不是泛化 agent 平台。
...
- 阶段三底座已进入主线，但 read-only、shadow 和非 assignment lease 仍有明确实现边界。

```yaml
ai_summary:
  authority: "仓库当前定位、主线能力、推荐启动方式和入口索引"
  scope: "控制平面主线、Trae-first 路径、运行入口、文档入口和发布包定位"
  read_when:
...
- dispatcher 现在支持可回滚的 Postgres / queue shadow path：`DISPATCHER_SHADOW_MODE=shadow-write` 与 `DISPATCHER_POSTGRES_URL` 打开后，SQLite 仍是真相源，外部库只承接 best-effort shadow projection。
- dispatcher 现在额外暴露 `GET /api/slo` 和 `GET /api/dr/status`，用于读取 burn-rate、只读状态、projection 健康度和备份清单。
- `DISPATCHER_READ_ONLY_MODE=1` 是当前只读降级开关，但它依赖 `dispatcher-server.ts:isMutationRequest` 的路由识别；在 matcher 补齐前，不应把它描述成完整写冻结。
- 仓库现在内置阶段三参考部署和演练入口：`pnpm verify:stage3`、`scripts/backup-runtime-state.mjs`、`scripts/restore-runtime-state.mjs`、`scripts/verify-stage3-dr.mjs`、`deploy/compose/*`、`deploy/helm/forgeflow/*`；源码备份/恢复脚本使用位置参数，打包 runtime CLI 才使用 `--backup-dir` 形式。
- `packages/provider-registry` 现在把 `trae` 作为正式 provider capability 记录在 registry 中；阶段三核心平台已完成，但对外 API / SDK / 第三方准入仍后置。
...
- 旧的本地 codex drill 脚本 `run-codex-control-flow.*`、`create-two-codex-drill-planner.*`、`run-dispatch-assignments.*`、`process-worker-result.*` 已退役；deferred 的 `codex/gemini` 只保留 `run-worker-daemon.js` 和其执行依赖。
