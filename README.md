...
- Trae automation gateway 的 route/session/chat handler、HTTP JSON IO、debug logger 和持久化 session-store 已下沉到 `@tingrudeng/automation-gateway-core`；源码脚本和 packaged runtime 只保留 driver 与时间格式 / 默认目录薄适配。
...
- `scripts/lib/*.js` 仍是 live adapter / bootstrap 层；dispatcher 的 server/state/review-memory/task-worktree 权威实现已下沉到 `apps/dispatcher`，Trae gateway 的 route/session/chat handler、HTTP server、debug logger 和持久化 session-store 已委托 `@tingrudeng/automation-gateway-core`，脚本层只保留 driver/time adapter 和 worker / daemon glue。
